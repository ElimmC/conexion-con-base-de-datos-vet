...
delimiter **
create procedure spGuardaCliente(in perID int, in nom nvarchar(120), in aps nvarchar(120))
begin
declare IdPer int;
declare existe int;
...
if perID = 0 then
    ...
    if (existe = 0)  then
		...
		insert into personas (personaId, nombre, apellidos, fecreg)
					values(IdPer, nom, aps, current_timestamp());
...
call spGuardaCliente(0, 'tarzan2', 'Osiris x2');
